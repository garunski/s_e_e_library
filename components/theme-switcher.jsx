...
export function ThemeSwitcher() {
  ...
  return (
    <div className="see-theme" role="group" aria-label="Theme">
      {OPTIONS.map((option) => {
        ...
        return (
          <button
            key={option.value}
            type="button"
            className={`see-theme__opt${active ? " see-theme__opt--active" : ""}`}
            aria-label={`Switch to ${option.value} theme`}
            aria-pressed={active}
            onClick={() => setTheme(option.value)}
          >
            <Icon name={option.value} />
            {active && <span className="see-theme__label">{option.label}</span>}
          </button>
        );
      })}
    </div>
